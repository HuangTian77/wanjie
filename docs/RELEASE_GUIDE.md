...
**创作（编辑器）**：三模式（可视化/代码/MUD）+ 蓝图工作区（60+ 节点/多图/子图）+ 2D/3D 场景编辑器（拖拽/撤销/快捷键/对齐）+ 模板系统（6 游戏类型）+ 剧本统计 + 校验跳转 + 查找替换 + 多标签页 + 封面/克隆/导出分享（内嵌封面 .json）

**游玩（体验器）**：事件驱动剧情（传统+蓝图双轨）+ 经济/战斗引擎（战斗 UI/奖励/技能 MP）+ 对话 + 酒馆 AI 对话 + 存档系统（继续游戏/自动存档/多槽位/回收站）+ HUD（金币/物品/角色/时钟）+ 评分

**社区（本地闭环）**：发布（published 状态）→ 市场标签页 → 收藏 → 分享文件导入导出 + 成就系统 + 个人统计

**系统**：设置（全屏/字号/动效/AI）+ 首启引导 + 操作帮助 + 导出构建（wanjie.exe）
