...
func _on_combat_started(enemies: Array) -> void:
	...
	# 战斗开始统计提示（累计胜场）
	if _battle_wins > 0:
		_battle_log_line("📊 当前战绩：胜 %d · 败 %d · 逃 %d" % [_battle_wins, _battle_defeats, _battle_flees], "#8a8278")
...
func _on_combat_ended(result: String) -> void:
	...
	if result == "victory":
		...
		if wins_after == 10:
			...
			if GameManager.user_data != null:
				GameManager.user_data.unlock_achievement("battle_10", "累计 10 场战斗胜利")
		elif wins_after == 50:
...
func _on_menu_log_pressed() -> void:
	...
	export_log.pressed.connect(func():
		...
		if f2:
			f2.store_string("万界世界日志导出\n")
			f2.store_string("时间：%s\n" % Time.get_datetime_string_from_system())
			f2.store_string("剧本：%s\n" % (script_data.name if script_data else "未知"))
			f2.store_string("%s\n\n" % "=".repeat(24))
...
- 成就解锁（战斗/评分/收藏/通关）"""
	...
	help_lbl.text = help_txt + "\n\n[color=#8a8278]万界 · v%s（体验版）[/color]" % ProjectSettings.get_setting("application/config/version", "1.0.0")
	# 字体大小随设置
	var help_fs := 14
